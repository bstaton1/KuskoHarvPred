...
author: "_v0.0.0.9001; FOR DRAFT/FEEDBACK USE ONLY_"
...
<details>
  ...
  *  The weather
  ...
  1.  Enter the expected conditions of the proposed fishery opportunity (e.g., day, start/end times, Bethel Test Fishery index values, wind speed/direction) on the `r icon("sliders-h")` **Inputs** tab
...
reactive({
  ...
  rvals$comp_preds_adj = KuskoHarvEst:::smart_round(rvals$comp_preds/sum(rvals$comp_preds), 2)
})
```
...
update_user_sliders = function(input, rvals) {
  ...
  effort_error = KuskoHarvPred:::get_mape("effort", KuskoHarvData:::get_period(input$date)) * rvals$preds$effort
  total_cpt_error = KuskoHarvPred:::get_mape("total_cpt", KuskoHarvData:::get_period(input$date)) * rvals$preds$total_cpt
...
preds_kable = function(input, rvals) {
  ...
  model_preds = data.frame(
    ...
    model = c(prettyNum(model_effort, big.mark = ",", scientific = FALSE), model_total_cpt, KuskoHarvEst:::percentize(c(model_chinook_comp, model_chum_comp, model_sockeye_comp)), prettyNum(c(model_chinook_harv, model_chum_harv, model_sockeye_harv, model_total_harv), big.mark = ",", scientific = FALSE))
  )
  ...
  if (input$show_user_adjust_preds) {
    ...
    user_preds = data.frame(
      quantity = c("Drift Trips", "Salmon Catch/Trip", "Chinook Composition", "Chum Composition", "Sockeye Composition", "Chinook Harvest", "Chum Harvest", "Sockeye Harvest", "Total Harvest"),
      user_lwr = c(prettyNum(user_effort[1], big.mark = ",", scientific = FALSE), user_total_cpt[1], KuskoHarvEst:::percentize(c(user_chinook_comp, user_chum_comp, user_sockeye_comp)), prettyNum(c(user_chinook_harv[1], user_chum_harv[1], user_sockeye_harv[1], user_total_harv[1]), big.mark = ",", scientific = FALSE)),
      
      user_mid = c(prettyNum(user_effort[3], big.mark = ",", scientific = FALSE), user_total_cpt[3], KuskoHarvEst:::percentize(c(user_chinook_comp, user_chum_comp, user_sockeye_comp)), prettyNum(c(user_chinook_harv[3], user_chum_harv[3], user_sockeye_harv[3], user_total_harv[3]), big.mark = ",", scientific = FALSE)),
      
      user_upr = c(prettyNum(user_effort[2], big.mark = ",", scientific = FALSE), user_total_cpt[2], KuskoHarvEst:::percentize(c(user_chinook_comp, user_chum_comp, user_sockeye_comp)), prettyNum(c(user_chinook_harv[2], user_chum_harv[2], user_sockeye_harv[2], user_total_harv[2]), big.mark = ",", scientific = FALSE))
    )
...
<details>
  ...
  **Condition Data**
  ...
  _The content on this page is currently under construction. Please wait for a future version for this feature._
...
actionLink("show_hide_data_kable_filters", label = "Show/Hide Filters", icon = icon("filter"))

# UI elements for filtering the outcome data
hidden(
  div(
    id = "data_kable_filters",
    fillRow(
      flex = c(1,3),
      ...
      column(
        ...
        wellPanel(
          selectizeInput("data_kable_years", "Years", choices = 2016:2021, selected = 2016:2021, multiple = TRUE,
                         options = list(placeholder = "Select one or more years")),
          selectizeInput("data_kable_periods", "Periods",
                         choices = list("Period 1 (6/12-6/19)" = 1, "Period 2 (6/20-6/30)" = 2, "Period 3 (7/1-7/31)" = 3),
                         selected = c(1,2,3), multiple = TRUE, options = list(placeholder = "Select one or more periods")),
          actionButton("filter_periods_select_all", "Select All")
        )
      ),
      ...
      column(
        ...
        wellPanel(
          splitLayout(
            sliderInput("data_kable_effort", "Drift Trips", min = min(dat$effort), max = max(dat$effort),
                        value = range(dat$effort), width = "95%"),
            sliderInput("data_kable_total_cpt", "Catch/Trip", min = min(round(dat$total_cpt)), max = max(round(dat$total_cpt)),
                        value = range(round(dat$total_cpt)), width = "95%"),
            sliderInput("data_kable_chinook_comp", "Chinook Composition", min = min(round(dat$chinook_comp, 2)) * 100, max = max(round(dat$chinook_comp, 2)) * 100,
                        value = range(round(dat$chinook_comp, 2) * 100), post = "%", width = "95%")
          ),
          actionButton("filter_outcomes_select_all", "Select All")
        )
      )
    ),
...
observeEvent(input$show_hide_data_kable_filters, {
  toggle(id = "data_kable_filters")
})

# reset the time period filters to default when action button is clicked
observeEvent(input$filter_periods_select_all, {
  updateSelectizeInput(inputId = "data_kable_years", selected = 2016:2021, multiple = TRUE)
  updateSelectizeInput(inputId = "data_kable_periods", selected = c(1,2,3), multiple = TRUE)
})

# reset the outcome filters to default when action button is clicked
observeEvent(input$filter_outcomes_select_all, {
  updateSliderInput(inputId = "data_kable_effort", value = range(dat$effort))
  updateSliderInput(inputId = "data_kable_total_cpt", value = range(round(dat$total_cpt)))
  updateSliderInput(inputId = "data_kable_chinook_comp", value = range(round(dat$chinook_comp, 2) * 100))
})

```

```{r outcome-data-fn}
data_kable = function(period_keep, year_keep, effort_range, total_cpt_range, chinook_comp_range) {
  ...
  table_dat$start_time = KuskoHarvEst:::short_datetime(table_dat$start)
  table_dat$end_time = KuskoHarvEst:::short_datetime(table_dat$end)
  ...
  table_dat$chinook_comp = KuskoHarvEst:::percentize(table_dat$chinook_comp)
  table_dat$chum_comp = KuskoHarvEst:::percentize(table_dat$chum_comp)
  table_dat$sockeye_comp = KuskoHarvEst:::percentize(table_dat$sockeye_comp)
...
reactive({
  rvals$data_kable = data_kable(period_keep = input$data_kable_periods, year_keep = input$data_kable_years,
                                effort_range = input$data_kable_effort,
                                total_cpt_range = input$data_kable_total_cpt,
                                chinook_comp_range = input$data_kable_chinook_comp/100
  )
})

# render table
renderUI(HTML(rvals$data_kable))
...
observeEvent(rvals$data_kable, {
  nrows_data_kable = max(stringr::str_count(as.character(rvals$data_kable), "<tr>") - 2, 0)
  updateActionLink(inputId = "show_hide_data_kable_filters", label = paste0("Show/Hide Filters (", nrows_data_kable, "/", nrow(dat), " records shown)"))
})
```
...
loo_plot = function(response) {
  ...
  text(x = mp, y = ifelse(x_mpe < 0, x_mpe - ydiff * 0.035, x_mpe + ydiff * 0.035), labels = KuskoHarvEst:::percentize(x_mpe/100), col = "grey50", font = 3)
  ...
  usr = par("usr"); ydiff = diff(usr[3:4])
  text(x = mp, y = ifelse(x_mape < 0, x_mape - ydiff * 0.035, x_mape + ydiff * 0.035), labels = KuskoHarvEst:::percentize(x_mape/100), col = "grey50", font = 3)
  axis(side = 1, at = mp, labels = period_labels, las = 1, lwd = 2)
...
error_summary = cbind(error_summary[,1:2], apply(error_summary[,-c(1,2)], 2, KuskoHarvEst:::percentize))
...
The code for this tool can be found [here](https://github.com/bstaton1/KuskoHarvPred/blob/main/inst/rstudio/KuskoHarvPred-tool/KuskoHarvPred-tool.Rmd).
