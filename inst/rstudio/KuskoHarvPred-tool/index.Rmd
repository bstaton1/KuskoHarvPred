...
subtitle: "For Daily Lower River Drift Harvest Based on 2016 -- 2022 In-Season Data"
...
fillRow(
  ...
  column(
    ...
    wellPanel(
      ...
      actionButton(inputId = "use_avg_btf", label = paste0("Get 2016 — ", max(dat$year), " Average"), width = "100%")
    )
  ),
  ...
  div(
    ...
    fillRow(
      ...
      column(
        ...
        wellPanel(
          # splitLayout(
            sliderInput("resp_kable_effort", "Drift Trips", min = min(dat$effort), max = max(dat$effort),
                        value = range(dat$effort), width = "95%"),
            sliderInput("resp_kable_total_cpt", "Catch/Trip", min = min(round(dat$total_cpt)), max = max(round(dat$total_cpt)),
                        value = range(round(dat$total_cpt)), width = "95%"),
            sliderInput("resp_kable_chinook_comp", "Chinook Composition", min = min(round(dat$chinook_comp, 2)) * 100, max = max(round(dat$chinook_comp, 2)) * 100,
...
observeEvent(input$filter_outcomes_select_all, {
  updateSliderInput(inputId = "resp_kable_effort", value = range(dat$effort))
  updateSliderInput(inputId = "resp_kable_total_cpt", value = range(round(dat$total_cpt)))
  updateSliderInput(inputId = "resp_kable_chinook_comp", value = range(round(dat$chinook_comp, 2) * 100))
})
...
observeEvent(rvals$resp_kable, {
  ...
  updateActionLink(inputId = "show_hide_resp_kable_filters", label = paste0("Show/Hide Filters (", nrows_resp_kable, "/", nrow(dat), " records shown)"))
})
```
...
pred_kable = function(period_keep, year_keep) {
  ...
  ordered_pred_vars = c("hours_open", "p_before_noon", 
                        "weekend", "not_first_day", "total_btf_cpue",
  ...
  table_dat$not_first_day = ifelse(table_dat$not_first_day, "Yes", "No")
  
  # round numeric variables
  table_dat$total_btf_cpue = round(table_dat$total_btf_cpue)
  table_dat$mean_Nwind = round(table_dat$mean_Nwind, 1)
  table_dat$mean_Ewind = round(table_dat$mean_Ewind, 1)
    ...
    out = table_dat %>%
      kbl("html", row.names = FALSE,
          col.names = c("Year", "Period", "Date",
                        ...
                        "Total CPUE", "%Chinook", "%Chum", "%Sockeye", "N. Wind", "E. Wind"),
          align = "cclcccccccccc") %>%
      kable_styling(full_width = TRUE, bootstrap_options = c("condensed"), fixed_thead = TRUE) %>%
      collapse_rows(1:2) %>%
      column_spec(3, width = "65px") %>%
      add_header_above(c(" " = 7, "Bethel Test Fishery" = 4, " " = 2))
...
observeEvent(rvals$pred_kable, {
  ...
  updateActionLink(inputId = "show_hide_pred_kable_filters", label = paste0("Show/Hide Filters (", nrows_pred_kable, "/", nrow(dat), " records shown)"))
})
```
...
pred_vars = c("day", "hours_open", "p_before_noon", "weekend", "not_first_day", "total_btf_cpue", "chinook_btf_comp", "chum_btf_comp", "sockeye_btf_comp", "mean_Nwind", "mean_Ewind")
...
div(
  ...
  renderPlot(expr = {
    par(mar = c(3.5,3.5,1,1), mgp = c(1.5,0.025,0), tcl = 0, cex = 1.5, cex.axis = 0.9, cex.lab = 1.1)
    vars_biplot(
