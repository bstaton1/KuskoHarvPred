...
reactive({
  rvals$df = data.frame(
    ...
    mean_Nwind = KuskoHarvUtils::get_Nwind(speed = input$wind_speed, angle = rvals$wind_angle),
    mean_Ewind = KuskoHarvUtils::get_Ewind(speed = input$wind_speed, angle = rvals$wind_angle)
  )
})
...
tab = cbind(Variable = rownames(tab), Importance = KuskoHarvUtils::percentize(tab$tab))
tab %>%
kbl("html", align = "lr") %>%
...
```{r period-table}
counts = table(KuskoHarvUtils::get_period(dat$day))
...
loo_plot = function(response) {
  ...
  text(x = mp, y = ifelse(x_mpe < 0, x_mpe - ydiff * 0.035, x_mpe + ydiff * 0.035), labels = KuskoHarvUtils::percentize(x_mpe/100), col = "grey50", font = 3)
  ...
  usr = par("usr"); ydiff = diff(usr[3:4])
  text(x = mp, y = ifelse(x_mape < 0, x_mape - ydiff * 0.035, x_mape + ydiff * 0.035), labels = KuskoHarvUtils::percentize(x_mape/100), col = "grey50", font = 3)
  axis(side = 1, at = mp, labels = period_labels, las = 1, lwd = 2)
...
pred_v_obs_plot = function(var) {
  ...
  yhat = loo_preds[,var]
  yobs = dat[,var]
  period = KuskoHarvUtils::get_period(dat$day)
