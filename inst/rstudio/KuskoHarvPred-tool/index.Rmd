...
- "resources/KuskoHarvPred-logo.png"
- "resources/all-logo-grouped.png"
- "resources/cites.bib"
- "resources/revised-fig1.png"
- "resources/revised-fig2.png"
- "resources/revised-fig3.png"
- "resources/revised-fig4.png"
- "resources/revised-table1.png"
- "resources/revised-table2.png"
- "resources/revised-table3.png"
- "resources/revised-table4.png"
- "resources/revised-table5.png"
- "resources/revised-tableA1.png"
- "resources/revised-tableA2.png"
- "resources/scatter-1.png"
- "resources/scatter-2.png"
---
...
`r htmltools::tagList(htmltools::p(paste0('Daily Drift Gillnet Harvest Based on ', min(KuskoHarvPred:::fit_data$year), '-', max(KuskoHarvPred:::fit_data$year), ' In-Season Monitoring')), htmltools::img(src="resources/KuskoHarvPred-logo.png", align = "right", height = "140px", style = "margin-top: -120px"))`
:::
...
# allows !expr to be used in yaml header
...
# function to build the sonar percentile dropdown menus
get_sonar_percentiles = function(date, species) {
  
  # replace all past years in the date
  # for summarizing sonar stats on this date for all past years
  yrs = unique(fit_data$year)
  dates = rep(date, length(yrs))
  year(dates) = yrs
  
  # summarize the sonar counts for this species on this date for all past years
  counts = sapply(dates, function(d) {
    KuskoHarvData:::summarize_sonar(date = d, stat = paste0(tolower(species), "_count"), plus_minus = 0)
  })
  
  # calculate the sonar count percentiles
  count_percentiles = round(quantile(counts, c(0.1, 0.25, 0.5, 0.75, 0.9)))
  
  # create names for the percentiles
  x = paste0(prettyNum(count_percentiles, big.mark = ","), " (", stringr::str_replace(names(count_percentiles), "\\%", "th Percentile"), ")")
...
set_sonar_count = function(input, species) {
  
  # set the category name for this species
  cat_name = paste0(species, "_sonar_count_input_cat")
  ...
  if (input[[cat_name]] != "6") {
    out = names(get_sonar_percentiles(input$date, species))[as.numeric(input[[cat_name]])] |> 
      stringr::str_remove(",") |> 
      stringr::str_extract("^[:digit:]+") |> 
      as.numeric()
  } else {
    out = input[[paste0("entered_", species, "_sonar_count")]]
  }
  return(out)
}
```
...
*  In-river abundance and species composition as indexed by the Kuskokwim River sonar project
...
:::{.indent}
1.  Enter the expected conditions of the proposed fishery opener (day, start/end times, sonar counts) on the `r bs_icon("sliders")` **Inputs** tab
...
The sonar project is operated by the Alaska Department of Fish and Game.
...
*  Chinook, chum, and sockeye salmon (no other species are included)
...
::::::{.blue}
<details>
<summary>`r bs_icon("exclamation-triangle")` Information for Readers of the NAJFM Article</summary>

`r hr()`

::::{.indent}
Some users of the tool have been directed to this webpage because it is linked in the publication:

:::{.indent style="padding-left: 10px; padding-right: 50px; text-indent: -20px"}
Staton, B. A.,  W. R. Bechtol, L. G. Coggins Jr., and G. Decossas. 2025. In-season predictions of daily harvest for Kuskokwim River subsistence salmon fisheries. _North American Journal of Fisheries Management._ 45(5): 779-794. doi: [10.1093/najfmt/vqaf055](https:/doi.org/10.1093/najfmt/vqaf055).

:::

There have been some changes to the model since publication -- notably several predictor variables and years -- that are documented in this section. 

::::

:::{.describe}
`r bs_icon("question-circle")` _What years have changed?_
:::

:::{.indent}

The analysis documented in the publication included data from **2016-2023** and spanned **40** individual days of fishing (i.e., "openers").

The revised analysis included in this tool includes data from **`r paste0(min(KuskoHarvPred:::fit_data$year), '-', max(KuskoHarvPred:::fit_data$year))`** and spans **`r nrow(KuskoHarvPred:::fit_data)`** openers.


The additional years of data collection have occurred since publication, whereas the removal of 2016 and 2017 is described in the next section.

:::

:::{.describe}
`r bs_icon("question-circle")` _What variables have changed, and why?_
:::

:::{.indent}
The most important change to the model stems from the discontinuation of the Bethel Test Fishery (BTF) following the 2024 season, upon which several predictor variables were based:

*  **Total BTF CPUE** -- a daily index of total in-river salmon abundance passing Bethel
*  **% BTF Chinook salmon** -- the daily proportion that Chinook salmon made up of BTF CPUE
*  **% BTF chum salmon** -- the daily proportion that chum salmon made up of BTF CPUE
*  **% BTF sockeye salmon** -- the daily proportion that sockeye salmon made up of BTF CPUE
*  **Quadratic versions** of these variables

These variables were influential in the predictive ability of the model, and since it is unlikely the BTF will resume operations in the future, a replacement was needed.
The replacement data set chosen was the Kuskokwim River sonar project, operated by the Alaska Department of Fish and Game. 
The sonar project was discussed in the Staton et al. (2025) article (pages 781; 792), where it was described that it was not used initially because data were not available for 2016 and 2017.
However, with the discontinuation of BTF operations, its usage became mandatory.

Thus, new variables based on the sonar were added as replacements of the BTF variables:

*  **Total sonar count** -- a daily count (estimate) of total in-river salmon abundance passing the sonar site (located several miles upstream of Bethel)
*  **% sonar Chinook salmon** -- the daily proportion that Chinook salmon made up of sonar counts
*  **% sonar chum salmon** -- the daily proportion that chum salmon made up of sonar counts
*  **% sonar sockeye salmon** -- the daily proportion that sockeye salmon made up of sonar counts
*  **Quadratic versions** of these variables

All places where the published analysis used a BTF variable, the corresponding sonar variable is now used as a replacement (e.g., total sonar count replaced total BTF CPUE).
The previous BTF variables were "smoothed" by taking the average of 3 consecutive days, centered on the day in question.
Because the sonar is expected to have lower day-to-day variability in catchability, the sonar variables were not smoothed in this fashion.

Inclusion of the sonar data set required that 2016 and 2017 be excluded from the analysis.
As a result the "fished yesterday" variable -- which indicated whether fishing occurred the previous day and described meaningful variability in the trips per day and catch per trip response variables -- was no longer needed (only applied to openers in 2016) and was been dropped from the analysis. 

:::

:::{.describe}
`r bs_icon("question-circle")` _Are there major differences in the results reported?_
:::
...
_Response Variable: Trips/Day_

:::{.indent}

Day and day^2^ remained important predictors of the trips/day response variable.
The removal of the "fished yesterday" variable is a notable difference between the published and revised analysis -- it previously explained much of the variability in trips/day in 2016, but with the removal of that year from the re-analysis, it is no longer used in the trips/day model.
The number of hours the fishery was open each day -- and the portion of that time that occurred before noon -- were also previously important predictors present in the top 5 models in the published analysis, but no longer came out as important in the revised analysis; this is potentially due to the lack of contrast, particularly 2016 where longer 24 hour openers were more prevalent than in the 2018-2025 years.
The index of total in-river abundance (BTF total CPUE in published analysis, total sonar count in revised analysis) was similarly unimportant across analyses, however, the index of % Chinook composition was present in the top model of the revised analysis and had a positive effect on effort, whereas in the published analysis it had a negligable effect.

:::

_Response Variable: Catch/Trip_

:::{.indent}

Similar to trips/day, the "fished yesterday" variable was an important predictor of catch/trip in the published analysis and is no longer present.
Beyond that, day and day^2^ remained approximately as important, as did the index of total in-river abundance.

:::

_Response Variables: Species Composition_

:::{.indent}

The models for chum and sockeye salmon remained largely unchanged: the day, day^2^, and sonar composition variables served similar roles and had similar effect sizes in the revised analysis as in the published analysis.
For the Chinook salmon composition model, however, the composition of Chinook salmon in the sonar was a not as good a predictor of the composition in the fishery as was the BTF, illustrated by smaller effect sizes and absence from the top model in the revised analysis.
...
Care should be taken in drawing strong inferences from the comparison of the leave-one-out predictive performance of the revised models against the published models, as there are differences in the years/openers included in each.
A perfect comparison would be to limit both analyses to only include 2018-2024 when both the BTF data and sonar data were available, however, such a perfect comparison is not needed since there is no choice but to abandon the BTF and move to the sonar data.

That said, the correlations in leave-one-out (LOO) predictions vs. actual outcomes were similar, especially when aggregated across all periods in the season (Table 3).
Further, the magnitude of bias (MPE) and precision (MAPE) scores were similar when aggregated across all periods to those presented in the published analysis (Table 4).
...
The following figures/tables show the same content as displayed in the published article, but revised to include the re-analysis with sonar data in place of BTF data for the years 2018-2025.
...
<figure align="center">
<img src="resources/revised-fig1.png" height="35%" width = "35%"/>
<figcaption>
Revised version of Fig. 1 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-fig2.png" height="35%" width = "35%"/>
<figcaption>
Revised version of Fig. 2 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation. Panels (a) and (b) exclude the "fished yesterday" dimension, since this variable was discarded from the re-analysis. Further, the three lines in panel (a) now display quantiles of the sonar % Chinook composition variable.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-fig3.png" height="80%" width = "80%"/>
<figcaption>
Revised version of Fig. 3 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-fig4.png" height="80%" width = "80%"/>
<figcaption>
Revised version of Fig. 4 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-table1.png" height="100%" width = "100%"/>
<figcaption>
Revised version of Table 1 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-table2.png" height="80%" width = "80%"/>
<figcaption>
Revised version of Table 2 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-table3.png" height="50%" width = "50%"/>
<figcaption>
Revised version of Table 3 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-table4.png" height="80%" width = "80%"/>
<figcaption>
Revised version of Table 4 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-table5.png" height="60%" width = "60%"/>
<figcaption>
Revised version of Table 5 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-tableA1.png" height="80%" width = "80%"/>
<figcaption>
Revised version of Table A1 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article supplemental material for details on interpretation.
</figcaption>
</figure>
...
<figure align="center">
<img src="resources/revised-tableA2.png" height="70%" width = "70%"/>
<figcaption>
Revised version of Table A2 from Staton et al. (2025) based on re-analysis with sonar data; see caption in article supplemental material for details on interpretation.
</figcaption>
</figure>
...
:::{.describe}
Enter the conditions of the proposed opener, including when it would occur and what the Kuskokwim River sonar counts have been like this year.
:::
...
**Expected Sonar Counts**

Next, select the expected daily Kuskokwim River sonar counts for the proposed fishing day.
For each species, pick either one of the 10^th^, 25^th^, 50^th^ (median), 75^th^, or 90^th^ percentiles, or choose to enter your own count value.
A **count percentile** (for example, 25^th^) is the count value that 25% of years (since 2018) fell below for the selected date.
Selecting the 50^th^ percentile means that counts have been approximately average this year, whereas selecting the 90^th^ percentile means counts have been much higher than average.

_Because the sonar count for the day of the opener you are making predictions for has not happened yet, insert your best expectation for what the sonar count will be._
:::
...
max_date = KuskoHarvUtils::from_days_past_may31(day = max(fit_data$day)); year(max_date) = year
...
```{r input-widgets}
# set the tooltip text for all manual entry sonar count boxes
enter_count_ttip_text = "The model uses sonar count values as an index of in-river abundance and species composition, so try not to enter extreme high or low values — just what you expect the sonar count to be for the selected day."
...
fillRow(
  ...
  column(
    ...
    wellPanel(
      ...
      selectInput(
        ...
        choices = start_time_bank, selected = "7:00AM"
      ),
      selectInput(
        inputId = "end_time", label = strong(bs_icon("clock"), "End Time"),
        choices = end_time_bank, selected = "7:00PM"
      )
  ...
  # input widgets for sonar count info
  column(
    width = 12,
    h4(strong("Expected Sonar Counts")),
    wellPanel(
      
      # describe why user needs to enter this and what percentiles are
      helpText(
        ttip("Why is this Important", "Fishery catch/trip and species composition are related to the total sonar count and species breakdown — the model uses these to predict outcomes.", icon_use = bs_icon("question-circle")),
        ttip("Percentiles", markdown("A 25<sup>th</sup> count percentile is the sonar count value that 25% of years (since 2018) fell below for the selected date. The 50<sup>th</sup> percentile is also called the median."), icon_use = bs_icon("question-circle"))
      ),
      
      hr(),
      
      # input widget for Chinook sonar count selections (based on pre-calculated percentiles)
      selectInput(
        inputId = "chinook_sonar_count_input_cat", label = strong(icon("fish-fins"), "Chinook Salmon Count"),
        choices = get_sonar_percentiles(date = default_date, species = "chinook"), selected = 3
      ),
      
      # input widget for Chinook sonar count manual entry
      hidden(
        div(
          id = "enter_chinook_sonar_count",
          numberInput(
            inputId = "entered_chinook_sonar_count", 
            label = ttip(strong("Enter Chinook Salmon Count"), enter_count_ttip_text),
            value = NULL, min = 0, placeholder = "Enter a count value"
          )
        )
      ),
      
      # input widget for chum sonar count selections (based on pre-calculated percentiles)
      selectInput(
        inputId = "chum_sonar_count_input_cat", label = strong(icon("fish-fins"), "Chum Salmon Count"),
        choices = get_sonar_percentiles(date = default_date, species = "chum"), selected = 3
      ),
      
      # input widget for chum sonar count manual entry
      hidden(
        div(
          id = "enter_chum_sonar_count",
          numberInput(
            inputId = "entered_chum_sonar_count",
            label = ttip(strong("Enter Chum Salmon Count"), enter_count_ttip_text),
            value = NULL, min = 0, placeholder = "Enter a count value"
          )
        )
      ),
      
      # input widget for sockeye sonar count selections (based on pre-calculated percentiles)
      selectInput(
        inputId = "sockeye_sonar_count_input_cat", label = strong(icon("fish-fins"), "Sockeye Salmon Count"),
        choices = get_sonar_percentiles(date = default_date, species = "sockeye"), selected = 3
      ),
      
      # input widget for sockeye sonar count manual entry
      hidden(
        div(
          id = "enter_sockeye_sonar_count",
          numberInput(
            inputId = "entered_sockeye_sonar_count", 
            label = ttip(strong("Enter Sockeye Salmon Count"), enter_count_ttip_text), 
            value = NULL, min = 0, placeholder = "Enter a count value"
          )
        )
      ),
      
      # where to find this year's sonar data
      helpText(
        a(bs_icon("table"), "View Sonar Count Data Webpage", href = "https://www.adfg.alaska.gov/index.cfm?adfg=commercialbyareakuskokwim.emihd")
      )
    )
  )
)
...
observeEvent(input$date, {
  updateSelectInput(
    inputId = "chinook_sonar_count_input_cat", selected = input$chinook_sonar_count_input_cat,
    choices = get_sonar_percentiles(date = input$date, species = "chinook")
  )
  updateSelectInput(
    inputId = "chum_sonar_count_input_cat", selected = input$chum_sonar_count_input_cat,
    choices = get_sonar_percentiles(date = input$date, species = "chum")
  )
  updateSelectInput(
    inputId = "sockeye_sonar_count_input_cat", selected = input$sockeye_sonar_count_input_cat,
    choices = get_sonar_percentiles(date = input$date, species = "sockeye")
  )
})

# if selected "Enter...", show box to enter
observe({
  toggle(id = "enter_chinook_sonar_count", condition = input$chinook_sonar_count_input_cat == 6, anim = TRUE, time = 0.3)
  toggle(id = "enter_chum_sonar_count", condition = input$chum_sonar_count_input_cat == 6, anim = TRUE, time = 0.3)
  toggle(id = "enter_sockeye_sonar_count", condition = input$sockeye_sonar_count_input_cat == 6, anim = TRUE, time = 0.3)
})
...
reactive({
  ...
  # select the correct sonar counts based on the inputs
  rvals$chinook_sonar_count = set_sonar_count(input, "chinook")
  rvals$chum_sonar_count = set_sonar_count(input, "chum")
  rvals$sockeye_sonar_count = set_sonar_count(input, "sockeye")
})
...
reactive({
  rvals$df = data.frame(
    ...
    total_sonar_count = rvals$chinook_sonar_count + rvals$chum_sonar_count + rvals$sockeye_sonar_count,
    chinook_sonar_comp = rvals$chinook_sonar_count/(rvals$chinook_sonar_count + rvals$chum_sonar_count + rvals$sockeye_sonar_count),
    chum_sonar_comp = rvals$chum_sonar_count/(rvals$chinook_sonar_count + rvals$chum_sonar_count + rvals$sockeye_sonar_count),
    sockeye_sonar_comp = rvals$sockeye_sonar_count/(rvals$chinook_sonar_count + rvals$chum_sonar_count + rvals$sockeye_sonar_count)
  )
})
...
# drop down menu options for sonar categories
CAT_sonar_choices = c(
...
fillRow(
  column(
    ...
    # input widget for the total sonar count category to show (hidden by default, shown if in global model for response variable)
    hidden(
      selectInput(inputId = "plot_CAT_total_sonar_count", label = strong("Total Sonar Count"), choices = CAT_sonar_choices, selected = "q50")
    ),
    
    # input widget for the sonar % Chinook category to show (hidden by default, shown if in global model for response variable)
    hidden(
      selectInput(inputId = "plot_CAT_chinook_sonar_comp", label = strong("Sonar % Chinook"), choices = CAT_sonar_choices, selected = "q50")
    ),
    
    # input widget for the sonar % chum category to show (hidden by default, shown if in global model for response variable)
    hidden(
      selectInput(inputId = "plot_CAT_chum_sonar_comp", label = strong("Sonar % Chum"), choices = CAT_sonar_choices, selected = "q50")
    ),
    
    # input widget for the sonar % sockeye category to show (hidden by default, shown if in global model for response variable)
    hidden(
      selectInput(inputId = "plot_CAT_sockeye_sonar_comp", label = strong("Sonar % Sockeye"), choices = CAT_sonar_choices, selected = "q50")
    )
  ),
...
observe({
  ...
  toggle(id = "plot_CAT_total_sonar_count", condition = "total_sonar_count" %in% rvals$vars)
  toggle(id = "plot_CAT_chinook_sonar_comp", condition = "chinook_sonar_comp" %in% rvals$vars)
  toggle(id = "plot_CAT_chum_sonar_comp", condition = "chum_sonar_comp" %in% rvals$vars)
  toggle(id = "plot_CAT_sockeye_sonar_comp", condition = "sockeye_sonar_comp" %in% rvals$vars)
})
...
reactive({
  rvals$plot_settings = list(
    ...
    CAT_total_sonar_count = input$plot_CAT_total_sonar_count,
    CAT_chinook_sonar_comp = input$plot_CAT_chinook_sonar_comp,
    CAT_chum_sonar_comp = input$plot_CAT_chum_sonar_comp,
    CAT_sockeye_sonar_comp = input$plot_CAT_sockeye_sonar_comp
  )
})
```
...
# e.g., default is to plot with mean sonar values, but could supply specific percentiles to see how this changes predicted curve
...
resp_kable = function(period_keep, year_keep, effort_range, total_cpt_range, chinook_comp_range) {
  ...
  if (nrow(table_dat) > 0) {
    out = table_dat |>
      ...
      row_spec(1:nrow(table_dat), extra_css = "border-bottom: 1px solid lightgray;")
...
pred_kable = function(period_keep, year_keep) {
  ...
  # format the year as a character, so it doesn't get a comma separator
  table_dat$year = as.character(table_dat$year)
  ...
  table_dat$chinook_sonar_comp = percentize(table_dat$chinook_sonar_comp)
  table_dat$chum_sonar_comp = percentize(table_dat$chum_sonar_comp)
  table_dat$sockeye_sonar_comp = percentize(table_dat$sockeye_sonar_comp)
  ...
  table_dat$total_sonar_count = round(table_dat$total_sonar_count)
    ...
    out = table_dat |>
      kbl("html", row.names = FALSE,
          col.names = c("Year", "Period", "Date", "Start Time", "End Time",
                        ...
                        "Total Count", "% Chinook", "% Chum", "% Sockeye"
          ),
          align = "cclllccccccc", format.args = list(big.mark = ","), 
      ...
      add_header_above(c(" " = 8, "Kuskokwim Sonar" = 4)) |> 
      row_spec(1:nrow(table_dat), extra_css = "border-bottom: 1px solid lightgray;")
...
pred_vars = c("day", "hours_open", "p_before_noon", "weekend", #"fished_yesterday",
              "total_sonar_count", "chinook_sonar_comp", "chum_sonar_comp", "sockeye_sonar_comp")
...
fillRow(
  ...
  column(
    ...
    selectInput(
      ...
      selected = "total_sonar_count", width = "100%"
    ),
...
tab = c(
  ...
  "Total Sonar Count",   "Total daily sonar count (Chinook+chum+sockeye)",                "YES", "YES", "NO" , "NO" , "NO" ,
  "Sonar % Chinook",     "Daily proportion that Chinook salmon made up of the total",     "YES", "NO" , "YES", "NO" , "NO" ,
  "Sonar % Chinook^2",   "Quadratic form; allows non-linear relationships",               "NO" , "NO" , "YES", "NO" , "NO" ,
  "Sonar % Chum",        "Daily proportion that chum salmon made up of the total",        "NO" , "NO" , "NO" , "YES", "NO" ,
  "Sonar % Chum^2",      "Quadratic form; allows non-linear relationships",               "NO" , "NO" , "NO" , "YES", "NO" ,
  "Sonar % Sockeye",     "Daily proportion that sockeye salmon made up of the total",     "NO" , "NO" , "NO" , "NO" , "YES",
  "Sonar % Sockeye^2",   "Quadratic form; allows non-linear relationships",               "NO" , "NO" , "NO" , "NO" , "YES",
...
tab = cbind(
  type = c(rep("Timing", 5), rep("Sonar", 7), rep("Weather<sup>a</sup>", 7), rep("Water<sup>b</sup>", 2)),
  # type = c(rep("Timing", 5), rep("Sonar", 7), rep("Weather", 7), rep("Water", 2)),
  tab
)
...
# create the table
kbl(tab, format = "html", escape = FALSE, align = "lllccccc", caption = vars_table_caption,
...
This involves averaging the predictions from each model so that those that are expected to produce better predictions carry more weight in the average.
...
This allows quantifying how the reliability of predictions changes throughout the season.
...
assign_color = function(p, allow_negatives) {
  ...
  if (allow_negatives) {
    ...
    breaks = c(-1,-0.40,-0.30,-0.20,-0.10,0.10,0.20,0.30,0.40,1)
  } else {
...
:::{.indent style="text-align: left; margin-left: 0px;"}
`r print_bibs(c("staton-coggins-2016", "staton-coggins-2017", "staton-2018", "decossas-2019b", "decossas-2020", "russell-etal-2021", "bechtol-schomogyi-2022", "bechtol-etal-2024", "bechtol-etal-2025", "bechtol-etal-2026"))`
:::
