...
fit_all_subsets_one = function(global_model, fit_data, parallel = FALSE, reduce_colinearity = FALSE, cwt_retain = 1) {
  ...
  if (reduce_colinearity) {
    # prevent sonar covariates from being in same model as day or day^2
    if ("total_sonar_count" %in% vars) M["total_sonar_count","day"] = FALSE
    if ("chinook_sonar_comp" %in% vars) M["chinook_sonar_comp","day"] = FALSE
    if (all(c("total_sonar_count", "I(day^2)") %in% vars)) M["total_sonar_count","I(day^2)"] = FALSE
    if (all(c("v", "I(day^2)") %in% vars)) M["chinook_sonar_comp","I(day^2)"] = FALSE

    # prevent sonar spp comp and total count from being in the same model
    if (all(c("chinook_sonar_comp", "total_sonar_count") %in% vars)) M["chinook_sonar_comp","total_sonar_count"] = FALSE
  ...
  # drop models that have chinook_sonar_comp^2 but not chinook_sonar_comp
  if("I(chinook_sonar_comp^2)" %in% vars) {
    dredge_out = subset(dredge_out, dc("chinook_sonar_comp", "I(chinook_sonar_comp^2)"))
  }

  # drop models that have chum_sonar_comp^2 but not chum_sonar_comp
  if("I(chum_sonar_comp^2)" %in% vars) {
    dredge_out = subset(dredge_out, dc("chum_sonar_comp", "I(chum_sonar_comp^2)"))
  }

  # drop models that have sockeye_sonar_comp^2 but not sockeye_sonar_comp
  if("I(sockeye_sonar_comp^2)" %in% vars) {
    dredge_out = subset(dredge_out, dc("sockeye_sonar_comp", "I(sockeye_sonar_comp^2)"))
  }
